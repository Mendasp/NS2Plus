...
//enchances stronger shade so it can adjust edge and model colour slightly differently on highlights	
	...
	if (fogShade == 0) {
		if (fogR == strongestColour){
			colourModel = colourFog * float4(1,colourMulti,colourMulti,0);
		}
		if (fogG == strongestColour){
			colourModel = colourFog * float4(colourMulti,1,colourMulti,0);
		}
		if (fogB == strongestColour){
			colourModel = colourFog * float4(colourMulti,colourMulti,1,0);
		}
	}
	else if (fogShade == 1) {
		if (fogB == strongestColour){
		...
		colourModel = colourFog * float4(1,1,colourMulti,0);
		}
	}
	else if (fogShade == 2) {
		if (fogG == strongestColour){
		...
		colourModel = colourFog * float4(1,colourMulti,1,0);
		}
	}
	else if (fogShade == 4) {
		if (fogR == strongestColour){
		...
		colourModel = colourFog * float4(colourMulti,1,1,0);
		}
	}
	else{
...
//offset colour when models are at an angle to camera
	...
	float4 colourAngle = lerp(colourOne, colourTwo, .75) * .6 * angleBlend;
...
//set up original mode model colouring
	...
	float4 edgeModelColour = 
	(model * (0.5 + 0.1 * pow(0.1 + sin(time * 5 + intensity * 4), 2)) * clamp(fadedist*.1,.5,1)) * colourModel +
	((model * pow(edge,2)) * (colourFog * (clamp(fadedist *4,.25,1)))) +
	(model * pow(edge,2) * 5 + model * pow(edge,2.5) * 100) * (colourFog * clamp(fadedist * 1,2,10));
	
	float4 modelColour = mainModelColour;//lerp(mainModelColour, edgeModelColour, clamp(avAreaX + avAreaY, 0, 1));
...
//VIEWMODEL mask
	...
	myAlien = clamp(pow(myAlien,.1),0,1);
	...
	if (modeAV >= 1){
		if (modeAV > 1){
			if (modeAV > 2){
				...
				((normal.y * .3) * ((0.5 + 0.2 * pow(0.1 + sin(time * 5 + intensity * 3), 2)) * model * (colourModel * 10 * inputPixelold) * (colourModel * 2) * clamp(fadedist*20,1,3)) *.25) +
				(pow(clamp(pow(model * edge,2.2),0,1) * colourFog * 50 * (fadeoff*100 + 100),1.2)) * pow((edge + model),30)) * clamp(pow(1-realvm,12),0,1) +
